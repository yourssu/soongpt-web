...
import {
  TimetableMutationState,
  TimetableMutationStatus,
} from '@/pages/TimetableSelectionActivity/type';
...
export const TimetableList = ({ mutationState }: TimetableListProps) => {
  const { pop } = useFlow();
  ...
  const timetableSelection: Record<TimetableMutationStatus, TimetableSelection> = {
    ...
    success: {
      ...
      element: () => (
        <>
          {mutationState.data &&
            mutationState.data.result.timetables.map((timetable, index) => (
              <div
